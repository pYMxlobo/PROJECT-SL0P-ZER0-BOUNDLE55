...
@onready var grapplesound := $"../grapple"
@onready var ungrapplesound := $"../ungrapple"
...
func _handle_hook(delta: float) -> void:
	...
	player_body.velocity += pull_vector * (clamp(pull_speed * get_parent().momentum, pull_speed, 100)) * delta * 60
